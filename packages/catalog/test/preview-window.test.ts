...
describe("contextual preview windows", () => {
  it("preserves full overlap and eight bars on each side at unequal rates", () => {
    ...
    const outgoing: SetPlanEntry = {
      id: "out", trackId: "a", order: 0, sourceStartMs: 0, sourceEndMs: 200_000,
      timelineStartMs: 0, playbackRate: 0.98, gainDb: -2,
      transitionToNext: { id: "join", type: "phrase_mix", durationMs: overlap,
        outgoingCuePointId: null, incomingCuePointId: null, parameters: { targetBpm: 174 } },
    };
    const incoming = { ...outgoing, id: "in", trackId: "b", playbackRate: 1.02, transitionToNext: null };
    const preview = slicePreview({ outgoing, incoming }, 30_000);
    expect(preview.overlapMs).toBe(overlap);
    const context = 32 * 60_000 / 174;
    expect((preview.outgoing.sourceEndMs - preview.outgoing.sourceStartMs) / 0.98 - overlap).toBeCloseTo(context, 6);
    expect((preview.incoming.sourceEndMs - preview.incoming.sourceStartMs) / 1.02 - overlap).toBeCloseTo(context, 6);
    expect(preview.outgoing.sourceEndMs - overlap * 0.98).toBeCloseTo(outgoing.sourceEndMs - overlap * outgoing.playbackRate, 6);
  ...
  it("uses piecewise mapping for a version-2 middle-pair preview", () => {
    ...
    const outgoing: SetPlanEntry = {
      id: "out", trackId: "a", order: 0, sourceStartMs: 0, sourceEndMs: 200_000,
      timelineStartMs: 0, playbackRate: 174 / 175, gainDb: 0,
      transitionToNext: { id: "join", type: "phrase_mix", durationMs: overlap,
        outgoingCuePointId: null, incomingCuePointId: null,
        parameters: { targetBpm: 174, rateRegionsVersion: 2 } },
    };
    const incoming = { ...outgoing, id: "in", trackId: "b", playbackRate: 1, transitionToNext: null };
  ...
  it("agrees on the middle join coordinate between a three-track chain and the extracted pair", () => {
    ...
    const b: SetPlanEntry = {
      id: "b", trackId: "b", order: 1, sourceStartMs: 0, sourceEndMs: sourceMs,
      timelineStartMs: 0, playbackRate: rate, gainDb: 0,
      transitionToNext: { id: "j1", type: "phrase_mix", durationMs: overlap,
        outgoingCuePointId: null, incomingCuePointId: null, parameters: { rateRegionsVersion: 2 } },
    };
    ...
    const previewTailSource = preview.outgoing.sourceStartMs + outputPositionToSourceMs(sliced, sliced.at(-1)!.outputEndMs - overlap);
